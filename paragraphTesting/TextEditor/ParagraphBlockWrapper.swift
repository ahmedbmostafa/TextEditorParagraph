...
struct ParagraphBlockWrapper: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: CustomizedUITextView, context: Context) {
        ...
        if !context.coordinator.isUserTyping && uiView.attributedText != attributedText {
              let selectedRange = uiView.selectedRange
              uiView.attributedText = attributedText
              let newLength = uiView.attributedText.length
              if selectedRange.location <= newLength && selectedRange.location + selectedRange.length <= newLength {
                  uiView.selectedRange = selectedRange
              } else {
                  uiView.selectedRange = NSRange(location: min(selectedRange.location, newLength), length: 0)
              }
          }
